...
static int set_size_distribution (int value)
{
    if (0 > value) {
        return -1;
    }

    options.size_distribution = (uint8_t) value;
...
int process_options (int argc, char *argv[])
{
    ...
    static struct option long_options[] = {
            ...
            {"allgatherv-size-dist",    required_argument,  0,  'S'},
    ...
    while ((c = getopt_long(argc, argv, optstring, long_options, &option_index)) != -1) {
        ...
        switch(c) {
            ...
            case 'W':
                ...
                break;
            case 'S':
                if (set_size_distribution(atoi(optarg))) {
                    bad_usage.message = "Invalid Allgatherv Size Distribution";
                    bad_usage.optarg = optarg;
