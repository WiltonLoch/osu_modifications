...
do {                                                                    \
   ...
   if (0 != osu_errno) {                                                    \
       fprintf(stderr, "[%s:%d] CUDA call '%s' failed with %d: %s \n",  \
        __FILE__, __LINE__, #stmt, osu_errno, cudaGetErrorString(osu_errno));   \
       exit(EXIT_FAILURE);                                              \
   }                                                                    \
   assert(cudaSuccess == osu_errno);                                        \
} while (0)
#endif
...
do {                                                                    \
   hipError_t osu_errno = (stmt);                                           \
   if (0 != osu_errno) {                                                    \
       fprintf(stderr, "[%s:%d] ROCM call '%s' failed with %d: %s \n",  \
        __FILE__, __LINE__, #stmt, osu_errno, hipGetErrorString(osu_errno));    \
       exit(EXIT_FAILURE);                                              \
   }                                                                    \
   assert(hipSuccess == osu_errno);                                         \
} while (0)
#endif
